...
| 基础数据接口 | 角色权限划分封装UserContex上下文帮助类,基础信息表数据接口,用户登录区分iris客户登录,通过http接口调用第三方接口,并且获取用户大小组,通过大小组绑定关联到系统中,定时任务预警管理通过页面配置时间窗口，转成cron表达式通过定时任务，来统计obs上传失败率和iris关联失败率 |
